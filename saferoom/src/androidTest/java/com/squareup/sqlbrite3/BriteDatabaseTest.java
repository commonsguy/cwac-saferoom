...
//    Factory factory = new FrameworkSQLiteOpenHelperFactory();
  ...
  @Test public void loggerEnabled() {
    ...
    db.insert(TestDb.TABLE_EMPLOYEE, CONFLICT_NONE, TestDb.employee("john", "John Johnson"));
  ...
  @Test public void query() {
    db.createQuery(TestDb.TABLE_EMPLOYEE, TestDb.SELECT_EMPLOYEES).subscribe(o);
    o.assertCursor()
  ...
  @Test public void queryWithQueryObject() {
    db.createQuery(
      TestDb.TABLE_EMPLOYEE, new SimpleSQLiteQuery(TestDb.SELECT_EMPLOYEES)).subscribe(o);
    o.assertCursor()
      .hasRow("alice", "Alice Allison")
      .hasRow("bob", "Bob Bobberson")
      .hasRow("eve", "Eve Evenson")
  ...
  @Test public void queryMapToOneOrDefault() {
    TestDb.Employee employees = db.createQuery(
      TestDb.TABLE_EMPLOYEE, TestDb.SELECT_EMPLOYEES + " LIMIT 1")
      .mapToOneOrDefault(
        TestDb.Employee.MAPPER, new TestDb.Employee("wrong", "Wrong Person"))
      .blockingFirst();
    assertThat(employees).isEqualTo(new TestDb.Employee("alice", "Alice Allison"));
  }
  ...
  @Test public void badQueryCallsError() {
    ...
    db.createQuery(TestDb.TABLE_EMPLOYEE, "SELECT * FROM missing").safeSubscribe(o);
  ...
  @Test public void queryNotNotifiedWhenInsertFails() {
    ...
    db.insert(TestDb.TABLE_EMPLOYEE, CONFLICT_IGNORE, TestDb.employee("bob", "Bob Bobberson"));
    o.assertNoMoreEvents();
  }
  ...
  @Test public void queryObservesUpdate() {
    ...
    db.update(
      TestDb.TABLE_EMPLOYEE, CONFLICT_NONE, values, TestDb.EmployeeTable.USERNAME + " = 'bob'");
    o.assertCursor()
      .hasRow("alice", "Alice Allison")
  ...
  @Test public void queryNotNotifiedWhenUpdateAffectsZeroRows() {
    ...
    db.update(
      TestDb.TABLE_EMPLOYEE, CONFLICT_NONE, values, TestDb.EmployeeTable.USERNAME + " = 'john'");
    o.assertNoMoreEvents();
  }
  ...
  @Test public void queryMultipleTables() {
    db.createQuery(TestDb.BOTH_TABLES, TestDb.SELECT_MANAGER_LIST).subscribe(o);
    o.assertCursor()
  ...
  @Test public void queryMultipleTablesWithQueryObject() {
    db.createQuery(
      TestDb.BOTH_TABLES, new SimpleSQLiteQuery(TestDb.SELECT_MANAGER_LIST)).subscribe(o);
    o.assertCursor()
      .hasRow("Eve Evenson", "Alice Allison")
      .isExhausted();
  }
  ...
  @Test public void queryMultipleTablesObservesChanges() {
    ...
    db.insert(TestDb.TABLE_MANAGER, CONFLICT_NONE, manager(testDb.bobId, testDb.eveId));
    o.assertCursor()
      .hasRow("Eve Evenson", "Alice Allison")
  ...
  @Test public void queryMultipleTablesObservesChangesOnlyOnce() {
    ...
    List<String> tables = Arrays.asList(
      TestDb.TABLE_EMPLOYEE, TestDb.TABLE_MANAGER, TestDb.TABLE_EMPLOYEE);
    db.createQuery(tables, TestDb.SELECT_MANAGER_LIST).subscribe(o);
    o.assertCursor()
      .hasRow("Eve Evenson", "Alice Allison")
      .isExhausted();
    ...
    db.update(
      TestDb.TABLE_EMPLOYEE, CONFLICT_NONE, values, TestDb.EmployeeTable.USERNAME + " = 'eve'");
    o.assertCursor()
  ...
  @Test public void queryOnlyNotifiedAfterSubscribe() {
    Observable<Query> query = db.createQuery(
      TestDb.TABLE_EMPLOYEE, TestDb.SELECT_EMPLOYEES);
    o.assertNoMoreEvents();
  ...
  @Test public void executeSqlAndTrigger() {
    ...
    db.executeAndTrigger(TestDb.TABLE_EMPLOYEE,
  ...
  @Test public void executeSqlAndTriggerMultipleTables() {
    db.createQuery(TestDb.TABLE_MANAGER, TestDb.SELECT_MANAGER_LIST).subscribe(o);
    o.assertCursor()
      .hasRow("Eve Evenson", "Alice Allison")
      .isExhausted();
    final RecordingObserver employeeObserver = new RecordingObserver();
    db.createQuery(TestDb.TABLE_EMPLOYEE, TestDb.SELECT_EMPLOYEES).subscribe(employeeObserver);
    ...
    final Set<String> tablesToTrigger = Collections.unmodifiableSet(new HashSet<>(
      TestDb.BOTH_TABLES));
  ...
  @Test public void executeInsertAndTriggerMultipleTables() {
    ...
    db.createQuery(TestDb.TABLE_MANAGER, TestDb.SELECT_MANAGER_LIST).subscribe(managerObserver);
...
/*
  @Test public void transactionOnlyNotifiesOnce() {
    ...
    try {
      db.insert(TestDb.TABLE_EMPLOYEE, CONFLICT_NONE, TestDb.employee("john", "John Johnson"));
      db.insert(TestDb.TABLE_EMPLOYEE, CONFLICT_NONE, TestDb.employee("nick", "Nick Nickers"));
      o.assertNoMoreEvents();
...
/*
  @Test public void synchronousQueryDuringTransaction() {
    ...
    try {
      transaction.markSuccessful();
      assertCursor(db.query(TestDb.SELECT_EMPLOYEES))
        .hasRow("alice", "Alice Allison")
        .hasRow("bob", "Bob Bobberson")
        .hasRow("eve", "Eve Evenson")
        .isExhausted();
...
/*
  @Test public void nestedTransactionsOnMultipleTables() {
    ...
    try {
      ...
      try {
        db.insert(
          TestDb.TABLE_MANAGER, CONFLICT_NONE, manager(testDb.aliceId, testDb.bobId));
        transactionInner.markSuccessful();
      } finally {
        transactionInner.end();
      }
  ...
  @Test public void nonExclusiveTransactionWorks() throws InterruptedException {
    ...
    new Thread() {
      @Override public void run() {
        ...
        try {
          db.insert(TestDb.TABLE_EMPLOYEE, CONFLICT_NONE, TestDb.employee("hans", "Hans Hanson"));
...
*/
  ...
  @Test public void badInsertThrows() {
    try {
      db.insert("missing", CONFLICT_NONE, TestDb.employee("john", "John Johnson"));
      fail();
    } catch (Exception e) {
      assertThat(e.getMessage()).contains("no such table: missing");
    }
  ...
  @Test public void badUpdateThrows() {
    try {
      db.update("missing", CONFLICT_NONE, TestDb.employee("john", "John Johnson"), "1");
      fail();
    } catch (Exception e) {
      assertThat(e.getMessage()).contains("no such table: missing");
    }
  ...
  private static RecordingObserver.CursorAssert assertCursor(Cursor cursor) {
    return new RecordingObserver.CursorAssert(cursor);
  }
}
